...
class SyncSettingsService {
  ...
  static const _onlyWifiKey = 'sync_only_wifi';
  ...
  Future<SyncSettings> getSettings() async {
    ...
    final onlyWifi = prefs.getBool(_onlyWifiKey) ?? true;
  ...
  Future<void> saveSettings(SyncSettings settings) async {
    ...
    await prefs.setBool(_onlyWifiKey, settings.onlyWifi);
    await prefs.setString(_folderKey, settings.folder);
    await prefs.setInt(
        _startDateKey, settings.startDate.millisecondsSinceEpoch);
  ...
  Future<void> clearSettings() async {
    ...
    await prefs.remove(_onlyWifiKey);
